...
function replaceData(template,  data, fields){
    ...
    //в цикле заменяем все заглушки в шаблоне
...
function insertCountGoods(){
    let url= "http://localhost:8000/app/getCount";
    let xhr = new XMLHttpRequest();
    xhr.open("GET", url, false);
    xhr.send();
...
function issetToken(){
    ...
    if(token != ""){
        ...
        let xhr = new XMLHttpRequest();
        xhr.open("POST", "http://localhost:8000/app/check_tn", false)
        ...
        if(response.status == true){
            ...
            document.querySelector(".log_basket-link").innerHTML = `<a href="/signin" class="log-link">Войти</a></span>`
            alert("no token")
